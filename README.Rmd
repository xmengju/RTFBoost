...
The depth of the base learners in `RTFBoost` is set with the argument `max.depth` in `control$tree.control`. We considered `max.depth` from 1 to 4, and chose the depth the minimizes the robust MSPE on the validation set at early stopping time. Denote validation residuals as $$r$$, the robust MSPE is defined as 


Below we train `RTFBoost` with `control$type = 'L2'` and select the depth `max.depth`.  This step may take several minutes to run:
