...
n.init <- 5
niter <- 10 # number of boosting iterations 
...
\end{equation}
$$, the robust MSPE is defined as $$\hat{\mu}_M( \{r_i, i \in \mathcal{I}_{\text{val}}\}) + \hat{\sigma}_M^2( \{r_i, i \in \mathcal{I}_{\text{val}}\}), $$
...
for(dd in 1:length(tree.depths)) {
   model.list.rr[[dd]] <- 
    RTFBoost.validation(x.train = xtrain,  y.train = ytrain,  x.val = xval,  y.val =  yval, x.test = xtest,  y.test = ytest, grid = grid, t.range = t.range, 
            ...
            tree.control =  TREE.control(tree.type = tree.type, num.dir = num.dir, max.depth = dd),
            shrinkage = shrinkage, nknot = nknot, type = type, init.type = "LADTree"), save.all = TRUE)
   val.errors[dd]  <- cal.rmspe(model.list.rr[[dd]]$f.val - yval)
...
for(dd in 1:length(tree.depths)) {
   model.list.rrm[[dd]] <- 
    RTFBoost(x.train = xtrain,  y.train = ytrain,  x.val = xval,  y.val =  yval, x.test = xtest,  y.test = ytest, grid = grid, t.range = t.range, 
...
predictions <- RTFBoost.predict(model.l2.separate, newx = xtest, grid = grid, t.range = t.range)
```
