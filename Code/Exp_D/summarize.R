...
summarize0 <- function(g_func_no, type, SNR = 5, nknot = 3, seeds = 1:20, summarize_type = "pred"){
  ...
  dir <- paste("Results_cedar10/Results_0_type_", type, "_g_", g_func_no, "_SNR_", SNR,"_nknot", nknot, sep = "")
...
summarize1 <- function(g_func_no, type, SNR = 5, nknot = 3, seeds =c(1:3, 5:20), summarize_type = "pred"){
  ...
  dir <- paste("Results_cedar10/Results_1_type_", type, "_g_", g_func_no, "_SNR_", SNR,"_nknot", nknot, sep = "")
  file_list <- list.files(dir)
  ...
  for(seed in seeds){
    ...
    for(d in 1:4){
      ...
      if(file_name %in% file_list){
        ...
        tmp_test <- rbind(tmp_test,  c(dat2save$RTFBoost$err_test[1:3],dat2save$RTFBoost$errs_test[1]))
    ...
    res <- rbind(res, tmp)
    res_early  <- rbind(res_early,  c( tmp_early[idx[1],1], tmp_early[idx[2],2], tmp_early[idx[3],3], tmp_early[idx[4],4]))
  }
  
  colnames(res) <- c("RTFBoost(L2)", "RTFBoost(LAD)", "RTFBoost(LAD-M)", "RTFBoost(RR)")
...
make_figure <- function(g_func_no, method_type = "RTFBoost"){
  ...
  for(type in c("C0","C1","C2","C3","C4","C5")){
    ...
    colnames(res1) <- c("TFBoost(L2)", "TFBoost(LAD)", "TFBoost(LAD-M)", "TFBoost(RR.2)", "type")
    tmp  <- rbind(tmp, res1)
  }
...
pp[[1]]
pdf("make_figures_10.pdf", width = 12, height = 8)
for(i in 1:5){
  print(pp[[i]])
}
dev.off()
