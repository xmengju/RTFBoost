set.control <- function(d, precision,  shrinkage.l2,shrinkage.lad, shrinkage.rr,   nknot, n_init, niter){
...
exp.tree.methods<- function(x_train, y_train, x_val, y_val, x_test, y_test,  grid, t_range, control.tree.list, validation_tree){
  ...
  err_trains <- err_tests <- err_vals <-  matrix(NA, length(control.tree.list), control.tree.list[[1]]$niter)
  ...
  return(list(errs_test = errs_test, params =   params, when_init = when_init,  err_cvs =  err_cvs , pred_train = res_trains, pred_val = res_vals, pred_test = res_tests, 
