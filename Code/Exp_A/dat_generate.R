...
dat.generate <- function(seed, control = dat.generate.control(), type = "C0", direction = "symmetric"){
  ...
  if(type == "C1"){
    ...
    #xi_matrix_out_train[,2] <- rnorm(length(outliers_train), mean = 5, sd =0.25)
    ...
    #xi_matrix_out_val[,2] <- rnorm(length(outliers_val), mean = 5, sd =0.25)
  ...
  if(type == "C2"){
   # tmpp_train <- rbinom(length(outliers_train),1,0.5)
  #  tmpp_val <- rbinom(length(outliers_val),1,0.5)
  ...
  if(type == "C0"){
    ...
    if(type %in% c("C1","C3","C4","C5", "C2")){
      if(direction == "asymmetric"){
        ...
        dat2return$y$y_train[outliers_train] <- dat2return$g$g_train[outliers_train] + tmp_train*rnorm(length(outliers_train), 5, 0.25)+
          (1-tmp_train)*rnorm(length(outliers_train), -10, 0.25)
        tmp_val <- rbinom(length(outliers_val), 1,0.5)
        dat2return$y$y_val[outliers_val] <- dat2return$g$g_val[outliers_val] + tmp_val*rnorm(length(outliers_val), 5, 0.25)+
          (1-tmp_val)*rnorm(length(outliers_val), -10, 0.25) 
      }
    }
    if(type == "C6"){
      dat2return$y$y_train[outliers_train] <- 10*dat2return$y$y_train[outliers_train] 
      dat2return$y$y_val[outliers_val] <- 10*dat2return$y$y_val[outliers_val] 
    }
  }
