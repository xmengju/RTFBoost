set.control <- function(d, precision,shrinkage, nknot, n_init, niter){
  ...
  return(list(control.l2 = control.l2,  control.lad = control.lad, control.lad.m = control.lad.m, control.rr.2 = control.rr.2, control.rr.5 = control.rr.5))
}
...
exp.tree.methods<- function(x_train, y_train, x_val, y_val, x_test, y_test,  grid, t_range, control.tree.list, validation_tree){
  ...
  for(i in 1:length(control.tree.list)){
    ...
    if(control.tree.list[[i]]$type == "RR" & control.tree.list[[i]]$bb == 0.5){
      when_init_rr.5 <- model.tree.list[[i]]$when_init
    }
    if(control.tree.list[[i]]$type == "RR" & control.tree.list[[i]]$bb == 0.2){
      when_init_rr.2 <- model.tree.list[[i]]$when_init
    }
  ...
  return(list(loss_vals = loss_vals, when_init_lad= when_init_lad, when_init_rr.5 = when_init_rr.5, when_init_rr.2 = when_init_rr.2, 
              pred_train = res_trains, pred_val = res_vals, pred_test = res_tests, 
              err_trains = err_trains, err_tests = err_tests, err_vals = err_vals, 
              err_test = err_test, err_val = err_val, early_stop = early_stop))
}
