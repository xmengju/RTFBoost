...
dat.generate <- function(seed, control = dat.generate.control(), type = "C0", direction = "symmetric"){
  ...
  sd_list <- readRDS("Code/Exp_E/sd_list.rds")
  sd_g <- sd_list[[control$g_func_no]]
  mattern_tmp <- readRDS("Code/Exp_E/mattern.rds")
  mu <-  mattern_tmp$mu
  phis <- mattern_tmp$phis
  K <- length(phis)
...
# }
# 
# saveRDS(sd_list, file = "Code/Exp_D/sd_list.rds")
