...
source("Code/utils.R")
source("Code/Exp_C/fun_exp_LAD.R")
...
conduct.exp <- function(exp_id = 1, conduct_sheet){
  ...
  for(i in 1:ncol(conduct_sheet)) {
    ...
    if(!is.na(conduct_sheet[exp_id,i])){
      ...
      if(test){
        dir_name <- paste("Results_tmp_LAD/Results_", case_id, "_type_", type, "_g_",g_func_no,  "_SNR_", SNR, "_nknot", nknot,  sep = "")
      }else{
        dir_name <- paste("Results_LAD/Results_", case_id, "_type_", type, "_g_",g_func_no, "_SNR_", SNR, "_nknot", nknot,  sep = "")
      }
